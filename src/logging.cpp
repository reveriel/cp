...
void Logger::setOutput(OutputFunc out) { g_outputFunc = out; }

void Logger::setFlush(FlushFunc flush) { g_flushFunc = flush; }
...
void Logger::Impl::formatTime() {
  ...
  char us_buf[32];
  snprintf(us_buf, sizeof(us_buf), ".%06d ", microseconds);
  stream_ << T(t_time, 17) << T(us_buf, 8);
}
